...
import { Send, User, Bot, MoreVertical, Database, BookOpen, FileText, LogOut, ChevronRight } from 'lucide-react';
...
export default function Dashboard() {
  ...
  // NEW: State to toggle the profile menu
        ...
        content: (
          <span>
            ...
            <ul className="list-disc pl-4 space-y-2 mb-3 text-slate-700">
              <li>Standard RAG relies on vector similarity, which can miss multi-hop reasoning [1].</li>
              <li>Graph RAG traverses connected nodes to find relevant context that may not be semantically similar but is structurally related [3].</li>
              <li>This reduces hallucinations by grounding answers in verifiable graph paths [2].</li>
            </ul>
          </span>
        ),
        citations: ['1', '3', '2']
  ...
  return (
    <div className="flex h-screen bg-slate-50 font-sans text-slate-900 overflow-hidden">
      ...
      {/* LEFT SIDEBAR */}
      <div className="w-16 md:w-20 bg-slate-900 flex flex-col items-center py-6 gap-6 z-30 flex-shrink-0">
        ...
        {/* Profile Button Container with Relative Positioning */}
        <div className="mt-auto mb-4 relative">
          <button 
            onClick={() => setIsProfileOpen(!isProfileOpen)}
            className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${isProfileOpen ? 'bg-blue-600 text-white shadow-lg' : 'bg-slate-700 text-slate-300 hover:text-white hover:bg-slate-600'}`}
            title="User Profile"
          >
            <User size={18} />
          </button>

          {/* NEW: POPUP PROFILE MENU */}
          {isProfileOpen && (
            <>
              ...
              {/* The actual menu */}
              <div className="absolute left-14 bottom-0 w-64 bg-white rounded-xl shadow-2xl border border-slate-200 p-2 z-50 animate-in slide-in-from-left-2 duration-200">
                <div className="p-3 border-b border-slate-100 mb-2">
                  <p className="text-sm font-bold text-slate-900">Dr. Research User</p>
                  <p className="text-xs text-slate-500">researcher@mbmc.edu.np</p>
                </div>
                
                <button 
                  onClick={() => setIsProfileOpen(false)}
                  className="w-full flex items-center justify-between p-2 text-sm text-slate-600 hover:bg-slate-50 rounded-lg mb-1 transition-colors"
                >
                ...
                <button 
                  ...
                  className="w-full flex items-center justify-between p-2 text-sm text-red-600 hover:bg-red-50 rounded-lg transition-colors font-medium"
                >
                  <span className="flex items-center gap-2">
              ...
              return (
                <div key={idx} className={`flex gap-3 ${isUser ? 'flex-row-reverse' : 'flex-row'} animate-in slide-in-from-bottom-2 duration-300`}>
                  ...
                  <div className={`flex flex-col max-w-[85%] ${isUser ? 'items-end' : 'items-start'}`}>
                    <div className={`px-5 py-4 rounded-2xl text-sm leading-7 shadow-sm ${isUser ? 'bg-blue-600 text-white rounded-tr-none' : 'bg-white border border-slate-200 text-slate-700 rounded-tl-none'}`}>
                      {msg.content}
                    </div>
